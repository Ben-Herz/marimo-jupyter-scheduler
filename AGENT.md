...
- **Read-only scheduler database fails silently**: reads keep working, so the dashboard looks healthy while every write path (job creation, YAML import, status updates) fails; the only symptom is a `create_job` traceback whenever a schedule next happens to fire. Fixed in `db_health.py`, which probes with a real `CREATE TABLE` inside a rolled-back transaction — checking file permissions alone is not enough, because SQLite also needs to create its journal in the *containing directory*. The result is logged at extension load and returned as `db_error` from the dashboard endpoint, which renders a banner. Note the probe uses stdlib `sqlite3` with `isolation_level=None`: under SQLAlchemy, pysqlite issues an implicit COMMIT before DDL and the probe table survives the rollback.
- **Extension `_load_jupyter_server_extension` blocks server startup on large/NFS filesystems**: if the workspace root is a large or slow NFS share, accessing `scheduler.db_url` or starting the watchdog `Observer` on the main thread during extension load causes the server to hang before binding its port — JupyterHub's `http_timeout` then kills the container. Fixed in `__init__.py` by deferring everything after handler registration to a background thread — `_load_jupyter_server_extension` now returns immediately and all DB/filesystem/watchdog work happens in a daemon thread.
